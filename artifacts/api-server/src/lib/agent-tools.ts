...
const ALLOWED_ROOT = path.join(PROJECT_ROOT, "artifacts", "engaja");
...
async function resolveSafePath(
  ...
  if (rel.startsWith("..") || path.isAbsolute(rel)) {
    return { ok: false, error: `Fora do escopo permitido (artifacts/engaja/): ${input}` };
  }
...
function buildShellCommand(input: {
  ...
  if (input.kind === "typecheck") {
    return {
      ok: true,
      cmd: "pnpm",
      args: ["--filter", "@workspace/engaja", "run", "typecheck"],
      cwd: PROJECT_ROOT,
    };
  }
  ...
  if (input.kind === "install_package") {
    return {
      ...
      cmd: "pnpm",
      args: ["add", pkg],
      cwd: path.join(PROJECT_ROOT, "artifacts", "engaja"),
    };
  }
